...
/** Mirrors WelcomeHomeFirstLaunch — only freeze during first-visit cinematic intro. */
function welcomeHomeArrivalFrozen(
  firstVisitCinematic: boolean,
  phase: "intro" | "pause" | "chat",
): boolean {
  return firstVisitCinematic && phase === "intro";
}
...
describe("Welcome Home returning user — experience integration", () => {
  ...
  it("returning user in chat phase does not freeze arrival (no black screen)", () => {
    const plan = evaluateWelcomeHomeExperience({ hasSeenWelcomeIntro: true });
    const firstVisitCinematic =
      plan.showIntro &&
      (plan.visitorKind === "first_visit" || plan.visitorKind === "replay");
    expect(firstVisitCinematic).toBe(false);
    expect(welcomeHomeArrivalFrozen(firstVisitCinematic, "chat")).toBe(false);
  });
