...
/** When true, chat never auto-opens workspaces — explicit member opens still fulfill (#183). */
...
Chat is CONVERSATION ONLY by default. You are the companion; the workspace is the tool; the user controls actions.
...
You MUST NOT auto-open tools or switch screens unless the member explicitly asked (open / start / take me to / create a …).
When they explicitly request a capability, fulfill it — Universal Access: rooms never block capabilities.
All other actions use explicit UI buttons — Build Draft, Save, Delete, Mark Done, Open Workspace, Add Item, Move Card, etc.
If the user asks you to perform a workspace write in chat without opening a tool, say warmly: "I can help you decide what to do. Use the button in the workspace when you're ready." Then help them think — do not claim you did the action.
...
/**
 * Conversation-only mode suppresses auto-routing.
 * Explicit workspace_open / tool_open outcomes are preserved (#183 Universal Access).
 */
export function enforceConversationOnlyTurnSurface(
  ...
  /** Explicit member opens must never be downgraded to chat_only. */
  if (surface.outcome === "workspace_open" || surface.outcome === "tool_open") {
