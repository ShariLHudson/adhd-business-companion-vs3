...
export const BREATHE_UNIVERSAL_PATTERN =
  /\b(?:help me breathe|let'?s breathe|i need (?:a minute|to breathe|a reset)|i need to calm down|calm me down|help me (?:calm down|reset)|i'?m overwhelmed|start(?:\s+a)?\s+breathing(?:\s+exercise)?|open(?:\s+the)?\s+breathe|take me to breathe|breathe(?:\s+and\s+reset)?|breathing exercise|box breathing|four[\s-]?seven[\s-]?eight|4[\s-]?7[\s-]?8(?:\s+breathing)?)\b/i;
