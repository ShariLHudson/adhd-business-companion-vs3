/**
 * 133 — Book Launch Event Blueprint foundation + certification.
...
describe("133 — Book Launch Event Blueprint foundation", () => {
  beforeEach(() => {
    resetWorkIdentityStoreForTests();
    resetWorkRelationshipsForTests();
    resetWorkBlueprintStateForTests();
  ...
  it("registers through the universal registry on event_plan only", () => {
    expect(isBlueprintRegistered(BOOK_LAUNCH_EVENT_BLUEPRINT_ID)).toBe(true);
    const bp = getBlueprint(BOOK_LAUNCH_EVENT_BLUEPRINT_ID)!;
    expect(bp.title).toBe("Book Launch");
    expect(bp.compatibleWorkTypeIds).toEqual([EVENT_PLAN_WORK_TYPE_ID]);
    expect(bp.category).toBe("spark");
    expect(EVENT_PLAN_BLUEPRINT_IDS).toContain(BOOK_LAUNCH_EVENT_BLUEPRINT_ID);
    expect(
      listBlueprints({ workTypeId: EVENT_PLAN_WORK_TYPE_ID }).some(
        (b) => b.blueprintId === BOOK_LAUNCH_EVENT_BLUEPRINT_ID,
      ),
    ).toBe(true);
  });
  ...
  it("package has no private runtime or durable store of its own", () => {
    ...
    for (const file of files) {
      ...
      expect(src).not.toMatch(/createPrivateBookLaunchRuntime|bookLaunchEventStore/);
      expect(src).not.toMatch(/localStorage\.setItem\(\s*["']book[_-]?launch/);
    }
  });
  ...
  it("conditional bookseller section appears with known context", () => {
    const init = initializeWorkFromBlueprint({
      workTypeId: EVENT_PLAN_WORK_TYPE_ID,
      blueprintId: BOOK_LAUNCH_EVENT_BLUEPRINT_ID,
  ...
  it("surfaces book launch forgotten items and adaptive purpose question", () => {
    const bp = getBlueprint(BOOK_LAUNCH_EVENT_BLUEPRINT_ID)!;
    expect(bp.commonlyForgottenItems.some((i) => /signing pens/i.test(i))).toBe(
      true,
    );
    expect(bp.commonlyForgottenItems.some((i) => /inventory/i.test(i))).toBe(
      true,
    );
  ...
  it("tasks and milestones use universal infrastructure", () => {
    ...
    addWorkMilestone({
      workId: init.workId,
      title: "Author goals and positioning approved",
    });
  ...
  it("research requires approve before apply", () => {
    ...
    const draft = createResearchRecord({
      ...
      researchQuestion: "Which bookstores host strong launch turnouts?",
      researchMode: "venue_scan",
      originatingExperience: "create",
      findings: "Independent bookstores with mailing lists convert better.",
    });
    ...
    const approved = createResearchRecord({
      ...
      researchQuestion: "Local media listing deadlines",
      researchMode: "quick_check",
      originatingExperience: "create",
      findings: "Weekly arts calendar needs copy 10 days ahead.",
      proposedActions: ["Submit arts calendar listing"],
    });
    submitResearchForReview(approved.id);
    approveResearch(approved.id);
    const applied = applyApprovedResearch(approved.id, [
      "Submit arts calendar listing 10 days ahead",
    ]);
  ...
  it("Project and cartography relationships attach to canonical Work", () => {
    ...
    linkWorkRelationship({
      ...
      toRef: { kind: "project", id: "proj-memoir-launch" },
  ...
  it("skip remains recoverable; answer advances purpose", () => {
    ...
    answerBlueprintQuestion(
      init.workId,
      "q_purpose",
      "Build a warm reader community around the new memoir",
    );
    expect(
  ...
  it("NL and Create launch resolve book launch; book signing stays distinct", () => {
    ...
    expect(inferred.blueprintId).toBe(BOOK_LAUNCH_EVENT_BLUEPRINT_ID);

    const signing = inferWorkTypeAndBlueprint({
      origin: "create",
      originalUserMessage: "Help me plan a book signing",
    });
    expect(signing.blueprintId).toBe("bp-event-book-signing");

    const product = inferWorkTypeAndBlueprint({
      origin: "create",
      originalUserMessage: "Help me plan a product launch",
    });
    expect(product.blueprintId).toBe(PRODUCT_LAUNCH_EVENT_BLUEPRINT_ID);

    const fromCreate = launchFromCreate({
      originalUserMessage: "Use the Book Launch Blueprint",
      candidateBlueprintId: BOOK_LAUNCH_EVENT_BLUEPRINT_ID,
    ...
    const talk = launchFromShari({
      originalUserMessage: "Just talk through this book launch with me",
      shariMode: "talk_only",
    });
    expect(talk.talkOnly).toBe(true);

    const fromShari = launchFromShari({
      originalUserMessage: "Continue the book launch I started",
  ...
  it("Chamber, Board, Body Doubling, and deliverables stay on universal owners", () => {
    ...
    expect(bp.chamberRoutingRecommendations).toEqual(
      expect.arrayContaining(["events", "content", "marketing"]),
    );
    expect(bp.boardReviewRecommendations).toEqual(
      expect.arrayContaining(["author goals", "book positioning"]),
    );
    ...
    const bd = launchFromOrigin("body_doubling", {
      originalUserMessage: "Body double with me while I pack inventory",
      relatedWorkId: workId,
      candidateWorkTypeId: EVENT_PLAN_WORK_TYPE_ID,
      bodyDoublingSessionId: "bd-bl-1",
      sectionId: "swag",
    });
    expect(bd.workId ?? workId).toBe(workId);

    const chamber = launchFromOrigin("chamber", {
      originalUserMessage: "Ask Content Intelligence to review the positioning",
      relatedWorkId: workId,
      chamberMemberId: "content",
      applyApproved: true,
    });
    expect(chamber.workId ?? workId).toBe(workId);

    const board = launchFromOrigin("board", {
      originalUserMessage: "Have the Board review the author goals",
      relatedWorkId: workId,
      boardReviewId: "br-bl-goals",
      applyApproved: true,
    });
    expect(board.workId ?? workId).toBe(workId);
  });

  it("duplicate prevention does not invent a second book launch Work", () => {
  ...
  it("Product Launch, Book Signing, and existing Event Blueprints remain registered", () => {
