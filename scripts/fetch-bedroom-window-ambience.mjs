#!/usr/bin/env node
/**
 * Download Bedroom Window™ ambience from a Songer share link.
 *
 * Usage:
 *   node scripts/fetch-bedroom-window-ambience.mjs
 *   node scripts/fetch-bedroom-window-ambience.mjs https://songer.co/song/xww1safasfl7vavpicfkabdx
 */
...
const OUT_FILE = path.join(OUT_DIR, "bedroom-window-ambience.mp3");

const DEFAULT_URL =
  "https://songer.co/song/xww1safasfl7vavpicfkabdx";
...
async function findAudioUrl(pageUrl) {
  ...
  throw new Error(
    "Could not find audio URL in Songer page. Save the MP3 as public/audio/peaceful-places/bedroom-window-ambience.mp3",
  );
}
