...
```mermaid
flowchart TD
  N[Notice] --> U[Understand] --> R[Research] --> T[Think]
  T --> C[Compare Options] --> REC[Recommend] --> FC[Founder Chooses]
  FC --> P[Prepare] --> AP[Approve] --> E[Execute]
  E --> M[Monitor] --> AD[Adjust] --> RV[Review] --> REM[Remember] --> IM[Improve]
  IM -.-> N
```
...
```mermaid
flowchart TB
  F[Founder Executive HQ]
  F -->|implementation plan| CU[Cursor]
  CU -->|code| GH[GitHub]
  F -->|content queue| PC[PostCraft]
  PC -->|publish after approval| GHL[GoHighLevel]
  GHL -->|campaigns| CUST[Customers]
  GH -->|releases| AN[Analytics]
  AN -->|signals| F
  F -->|delegation| TH[Team Hub]
  TH -->|tasks| IZ[Izna / Team]
  F -->|member intelligence| COMP[Spark Companion]
  COMP -->|feedback| F
```
