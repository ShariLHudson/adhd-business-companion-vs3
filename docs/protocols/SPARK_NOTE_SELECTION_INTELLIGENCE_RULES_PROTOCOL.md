...
| Protocol rule | Module |
|---------------|--------|
| Priority 1–2 personal + upcoming | `personalSparks.ts` → `resolvePersonalSpark()` |
| Priority 3 calendar / seasonal | `evaluateDailySparkNote.ts` → `resolveFromCatalog()` |
| Priority 4 affinity learning | `preferenceLearning.ts` → `scoreEntryAffinity()` |
| Priority 5 discovery + variety | `librarySelection.ts` → `filterLibraryCandidatePool()` |
| Priority constants + lookback | `selectionIntelligence.ts` |
| Conflict resolution (personal > holiday) | `evaluateDailySparkNote.ts` personal check before catalog |
| Tone-adapted personal cards | `personalSparks.ts` builders |
| Strong vs weak learning signals | `persistence.ts` reactions + `preferenceLearning.ts` caps |
| One spark per day | `persistence.ts` `dailySelection` + `evaluateDailySparkNote()` cache |

## Verify

```bash
npx vitest run lib/sparkNote/selectionIntelligence.test.ts lib/sparkNote/evaluateDailySparkNote.test.ts lib/sparkNote/preferenceLearning.test.ts
```
