...
| **Business context** (Business Profile / Client Avatar / industry) | ❌ **Do not add in V2** | N/A | Highest risk, lowest evidence of need. No pilot scenario in the review showed a case where business context would have resolved an ambiguity that language/intent/journey stage couldn't. Business Profile data can be stale (Spec 112's own "never assume observation = memory" rule), and using it to bias which *expert* activates risks a subtler failure mode than the one being fixed: quietly wrong activation the member can't see or correct, driven by profile data they may not remember providing. **Revisit only if a future review finds a concrete case business context would have fixed** — don't add it speculatively |
