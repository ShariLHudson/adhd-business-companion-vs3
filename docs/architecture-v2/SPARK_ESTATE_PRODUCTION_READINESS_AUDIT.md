...
**Create deep-dive:** [SPARK_ESTATE_CREATE_EXPERIENCE_INVENTORY_AND_GAP_AUDIT.md](./SPARK_ESTATE_CREATE_EXPERIENCE_INVENTORY_AND_GAP_AUDIT.md) — Menu shows ~29–30 creation types, but only **4** are registered guided work types (Event Plan, Marketing Plan, Business Plan, Facebook Community); ~26 are single-artifact content-generator drafts; 3 (SOP/Checklist/Proposal) are "phantom" work types (id resolves, no schema); **0** Production Ready (browser NOT_RUN). Extends §3.3 / P0-02/03/04.
...
| **P0-03** | C | Single creation runtime | **Ownership contract landed:** `lib/createEstate/createOwnershipContract.ts` — entry=`universalCreationEntrypoint`; work=`UWE`; UI=`createEstate`; lifecycle=`UCE`. `universalCreation` + `platformIntent/blueprintRegistry` marked adapter-only; legacy split shell remains quarantined. **2026-07-22:** Facebook Community (587–598) now registers on the live canonical path via UWE `index.ts` side-effect import + Anywhere-Origin `ensure` + FB inference/Blueprint attach — proven by `facebookCommunity.liveRegistration.test.ts` (import-only registration + NL routing). No new Create stack added. | **Partial → ownership enforced** (stacks not deleted) | Wrong room / duplicate Work / trust breaks | Retarget remaining openCreateWorkspace callers; do not expand adapter registries |
