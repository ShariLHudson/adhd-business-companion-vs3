/**
 * 135 — Browser checklist for Masterclass Event Blueprint (jsdom).
...
describe("135 Masterclass Event browser checklist", () => {
    ...
    act(() => {
      root.render(
        <UniversalBlueprintInterface
          ...
          knownContext={{
            purpose: "Help clients leave able to price their next offer",
            audience: "Service business owners ready for a deep dive",
          }}
          onWorkReady={(state) => {
  ...
  it("starts Masterclass Quick Start, preserves Work ID on Guided Build, paid context sticks", () => {
    click(
    ...
    click(
      container.querySelector(
        `[data-testid='ubi-blueprint-${MASTERCLASS_EVENT_BLUEPRINT_ID}']`,
      ),
    );
    expect(
    ...
    expect(state?.blueprintId).toBe(MASTERCLASS_EVENT_BLUEPRINT_ID);
    ...
    mergeKnownContext(workId, { is_paid: "true" });
    expect(getWorkBlueprintState(workId)?.knownContext.is_paid).toBe("true");
  });
});
