/**
 * 129 — Browser checklist for Retreat Event Blueprint (jsdom).
...
describe("129 Retreat Event browser checklist", () => {
    ...
    act(() => {
      root.render(
        <UniversalBlueprintInterface
          ...
          knownContext={{
            purpose: "Help the leadership team find one shared priority",
  ...
  it("starts Retreat Quick Start, preserves Work ID on Guided Build, volunteer context sticks", () => {
    click(
    ...
    click(
      container.querySelector(
        `[data-testid='ubi-blueprint-${RETREAT_EVENT_BLUEPRINT_ID}']`,
      ),
    );
    expect(
    ...
    expect(state?.blueprintId).toBe(RETREAT_EVENT_BLUEPRINT_ID);
    ...
    mergeKnownContext(workId, { needs_volunteers: "true" });
    expect(getWorkBlueprintState(workId)?.knownContext.needs_volunteers).toBe(
      "true",
    );
  });
});
