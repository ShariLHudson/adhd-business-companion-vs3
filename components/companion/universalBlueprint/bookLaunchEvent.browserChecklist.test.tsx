/**
 * 133 — Browser checklist for Book Launch Event Blueprint (jsdom).
...
describe("133 Book Launch Event browser checklist", () => {
    ...
    act(() => {
      root.render(
        <UniversalBlueprintInterface
          ...
          knownContext={{
            purpose: "Build a warm reader community around the new memoir",
            audience: "Local readers and book-club hosts",
          }}
          onWorkReady={(state) => {
  ...
  it("starts Book Launch Quick Start, preserves Work ID on Guided Build, bookseller context sticks", () => {
    click(
    ...
    click(
      container.querySelector(
        `[data-testid='ubi-blueprint-${BOOK_LAUNCH_EVENT_BLUEPRINT_ID}']`,
      ),
    );
    expect(
    ...
    expect(state?.blueprintId).toBe(BOOK_LAUNCH_EVENT_BLUEPRINT_ID);
    ...
    mergeKnownContext(workId, { has_bookseller: "true" });
    expect(getWorkBlueprintState(workId)?.knownContext.has_bookseller).toBe(
      "true",
    );
  });
});
