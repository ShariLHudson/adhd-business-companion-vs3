/**
 * 134 — Browser checklist for Challenge Event Blueprint (jsdom).
...
describe("134 Challenge Event browser checklist", () => {
    ...
    act(() => {
      root.render(
        <UniversalBlueprintInterface
          ...
          knownContext={{
            purpose: "Help busy founders take one focused action each morning",
            audience: "Founders who want a gentle daily rhythm",
          }}
          onWorkReady={(state) => {
  ...
  it("starts Challenge Quick Start, preserves Work ID on Guided Build, prizes context sticks", () => {
    click(
    ...
    click(
      container.querySelector(
        `[data-testid='ubi-blueprint-${CHALLENGE_EVENT_BLUEPRINT_ID}']`,
      ),
    );
    expect(
    ...
    expect(state?.blueprintId).toBe(CHALLENGE_EVENT_BLUEPRINT_ID);
    ...
    mergeKnownContext(workId, { has_prizes: "true" });
    expect(getWorkBlueprintState(workId)?.knownContext.has_prizes).toBe("true");
  });
});
