/**
 * 132 — Browser checklist for Product Launch Event Blueprint (jsdom).
...
describe("132 Product Launch Event browser checklist", () => {
    ...
    act(() => {
      root.render(
        <UniversalBlueprintInterface
          ...
          knownContext={{
            purpose: "Create trust and conversions for the new membership",
            audience: "Busy founders ready for a simpler membership",
          }}
          onWorkReady={(state) => {
  ...
  it("starts Product Launch Quick Start, preserves Work ID on Guided Build, affiliate context sticks", () => {
    click(
    ...
    click(
      container.querySelector(
        `[data-testid='ubi-blueprint-${PRODUCT_LAUNCH_EVENT_BLUEPRINT_ID}']`,
      ),
    );
    expect(
    ...
    expect(state?.blueprintId).toBe(PRODUCT_LAUNCH_EVENT_BLUEPRINT_ID);
    ...
    mergeKnownContext(workId, { has_affiliates: "true" });
    expect(getWorkBlueprintState(workId)?.knownContext.has_affiliates).toBe(
      "true",
    );
  });
});
