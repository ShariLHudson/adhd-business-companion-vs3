/**
 * 131 — Browser checklist for Summit Event Blueprint (jsdom).
...
describe("131 Summit Event browser checklist", () => {
    ...
    act(() => {
      root.render(
        <UniversalBlueprintInterface
          ...
          knownContext={{
            purpose: "Align partners on one shared growth priority",
            audience: "Partner executives",
          }}
          onWorkReady={(state) => {
  ...
  it("starts Summit Quick Start, preserves Work ID on Guided Build, partner context sticks", () => {
    click(
    ...
    click(
      container.querySelector(
        `[data-testid='ubi-blueprint-${SUMMIT_EVENT_BLUEPRINT_ID}']`,
      ),
    );
    expect(
    ...
    expect(state?.blueprintId).toBe(SUMMIT_EVENT_BLUEPRINT_ID);
    ...
    mergeKnownContext(workId, { has_partners: "true" });
    expect(getWorkBlueprintState(workId)?.knownContext.has_partners).toBe(
      "true",
    );
  });
});
