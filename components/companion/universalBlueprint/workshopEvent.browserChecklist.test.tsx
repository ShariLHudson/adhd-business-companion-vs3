/**
 * 127 — Browser checklist for Workshop Event Blueprint (jsdom).
...
describe("127 Workshop Event browser checklist", () => {
    ...
    act(() => {
      root.render(
        <UniversalBlueprintInterface
          ...
          knownContext={{
            purpose: "Help founders clarify their offer",
            outcomes: "Leave with a draft offer page",
          }}
          onWorkReady={(state) => {
  ...
  it("starts Workshop Quick Start, preserves Work ID on Guided Build, virtual context sticks", () => {
    click(
    ...
    click(
      container.querySelector(
        `[data-testid='ubi-blueprint-${WORKSHOP_EVENT_BLUEPRINT_ID}']`,
      ),
    );
    expect(
    ...
    expect(state?.blueprintId).toBe(WORKSHOP_EVENT_BLUEPRINT_ID);
    ...
    mergeKnownContext(workId, { is_virtual_or_hybrid: "true" });
    expect(
      getWorkBlueprintState(workId)?.knownContext.is_virtual_or_hybrid,
    ).toBe("true");
  });
});
