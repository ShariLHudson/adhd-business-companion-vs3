/**
 * 128 — Browser checklist for Webinar Event Blueprint (jsdom).
...
describe("128 Webinar Event browser checklist", () => {
    ...
    act(() => {
      root.render(
        <UniversalBlueprintInterface
          ...
          knownContext={{
            purpose: "Show founders a calm planning demo",
            outcomes: "Book a clarity call",
          }}
          onWorkReady={(state) => {
  ...
  it("starts Webinar Quick Start, preserves Work ID on Guided Build, recording context sticks", () => {
    click(
    ...
    click(
      container.querySelector(
        `[data-testid='ubi-blueprint-${WEBINAR_EVENT_BLUEPRINT_ID}']`,
      ),
    );
    expect(
    ...
    expect(state?.blueprintId).toBe(WEBINAR_EVENT_BLUEPRINT_ID);
    ...
    mergeKnownContext(workId, { will_record: "true" });
    expect(getWorkBlueprintState(workId)?.knownContext.will_record).toBe(
      "true",
    );
  });
});
