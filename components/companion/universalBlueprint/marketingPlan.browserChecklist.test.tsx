/**
 * 105 — Browser checklist for Simple Marketing Plan (jsdom).
...
describe("105 Marketing Plan browser checklist", () => {
  ...
  it("starts Simple Marketing Plan Quick Start and preserves Work ID on Guided Build", () => {
    click(
    ...
    click(
      container.querySelector(
        `[data-testid='ubi-blueprint-${MARKETING_PLAN_SIMPLE_BLUEPRINT_ID}']`,
      ),
    );
    expect(
    ...
    expect(
      container.querySelector("[data-testid='universal-blueprint-preview']")
        ?.textContent,
    ).toMatch(/Simple Marketing Plan/i);
    ...
    expect(state?.blueprintId).toBe(MARKETING_PLAN_SIMPLE_BLUEPRINT_ID);
    expect(state?.workTypeId).toBe(MARKETING_PLAN_WORK_TYPE_ID);
