...
import {
  ...
  listYearsFromDates,
  WINS_TIMELINE_GUIDING_LINE,
  WINS_TIMELINE_ITEM_TYPES,
} from "@/lib/estate/winsTimelineIntelligence";
...
export function MyJourneyPanel({
  ...
  const [timelineTypeFilter, setTimelineTypeFilter] = useState<string | null>(
    null,
  );
  ...
  const availableYears = useMemo(
    () => listYearsFromDates(entries.map((e) => e.date || e.createdAt)),
    [entries],
  );
  ...
  const visible = useMemo(() => {
    ...
    let next = filterEntriesByYear(base, yearFilter);
    if (timelineTypeFilter) {
      next = next.filter(
        (e) => journeyCategoryToTimelineType(e.category) === timelineTypeFilter,
      );
    }
    if (majorOnly) {
      next = next.filter((e) => isMajorTimelineMoment(e.category));
    }
    return next;
  }, [search, filter, entries, yearFilter, timelineTypeFilter, majorOnly]);
  ...
  function renderEntry(entry: JourneyEntry) {
    ...
    const timelineType = journeyCategoryToTimelineType(entry.category);
    return (
      <article
        key={entry.id}
      ...
      >
        ...
        >
          ...
          <div className="min-w-0 flex-1">
            <div className="flex flex-wrap gap-2 text-xs text-[#9a8f82]">
              ...
              {timelineType ? (
                <span className="rounded-full border border-[#e7d9c8] px-2 py-0.5 text-[#6f6259]">
                  {timelineType}
                </span>
              ) : null}
        ...
        {expanded ? (
          <div className="border-t border-[#efe8de] px-4 pb-4 pt-3 text-sm text-[#4b463f]">
            {entry.whatHappened ? (
              <p>
                <span className="font-semibold text-[#2f261f]">What happened: </span>
                {entry.whatHappened}
              </p>
            ) : null}
            {entry.whatDidILearn ? (
              <p className="mt-2">
                <span className="font-semibold text-[#2f261f]">What I learned: </span>
                {entry.whatDidILearn}
              </p>
            ) : null}
            {entry.howDidThisShapeMe ? (
              <p className="mt-2">
                <span className="font-semibold text-[#2f261f]">How it shaped me: </span>
                {entry.howDidThisShapeMe}
              </p>
            ) : null}
            {entry.whatWisdom ? (
              <p className="mt-2">
                <span className="font-semibold text-[#2f261f]">Wisdom: </span>
                {entry.whatWisdom}
              </p>
            ) : null}
            <GrowthAttachmentsList attachments={entry.attachments} />
            {entry.attachments.length > 0 ? (
              <p className="mt-2 text-xs text-[#6f6259]">
                Supporting evidence is attached above. You can also open the
                Evidence Vault for proof kept for harder days.
              </p>
            ) : (
              <button
                type="button"
                onClick={() => nav.onOpenSection("evidence-bank")}
                className="mt-3 text-xs font-semibold text-[#6f6259] underline-offset-2 hover:underline"
              >
                View supporting evidence in Evidence Vault
              </button>
            )}
            <button
              type="button"
              ...
              className="mt-4 block text-xs font-semibold text-[#9a6b6b]"
            >
              Remove entry
            </button>
          </div>
  ...
  return (
    <section className={workspacePanelShellClass({ width: "standard" })}>
      ...
      <p className="mt-2 text-sm text-[#6f6259]">{WINS_TIMELINE_GUIDING_LINE}</p>
      ...
      {stats.total > 0 ? (
        <details className="mt-5 rounded-2xl border border-[#e7d9c8] bg-[#faf7f2]/50 px-4 py-3">
          ...
          <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-4">
            ...
            <StatPill label="Wisdom" value={stats.wisdomEntries} />
          </div>
        </details>
      ) : null}

      {stats.total === 0 ? (
        <div className="mt-6 rounded-3xl border border-dashed border-[#e7d9c8] bg-white px-5 py-8 text-center">
          <p className="text-sm text-[#6f6259]">
            Your Wins Timeline is ready — a chronological story of growth over
            months and years.
          </p>
          <button
            type="button"
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        ...
        <button
          ...
          className={`rounded-full px-3 py-1.5 text-xs font-semibold ${
            majorOnly ? "bg-[#2f261f] text-white" : "border border-[#e7d9c8] bg-white"
          }`}
        >
          Major moments
        </button>
        <button
          ...
          className="rounded-full border border-[#e7d9c8] px-3 py-1.5 text-xs font-semibold text-[#6f6259]"
        >
          🖨 Print
        </button>
        <button
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  updateDraft(
                    "category",
                    e.target.value as JourneyEntryInput["category"],
                  )
                }
                className={INPUT_CLASS}
              >
            ...
            <div>
              ...
              <datalist id="journey-chapters">
                {JOURNEY_CHAPTER_PRESETS.map((c) => (
                  <option key={c} value={c} />
                ))}
                {chapters
                  .filter(
                    (c) =>
                      !(JOURNEY_CHAPTER_PRESETS as readonly string[]).includes(c),
                  )
                  .map((c) => (
                    <option key={c} value={c} />
                  ))}
      ...
      <div className="mt-6">
        <input
          ...
          placeholder="Search title, story, lessons, wisdom, attachments…"
        ...
        <div
          className="mt-2 flex flex-wrap gap-1"
          role="group"
          aria-label="Filter by timeline category"
        >
          <button
            type="button"
          ...
          {WINS_TIMELINE_ITEM_TYPES.map((t) => (
            <button
              key={t}
              type="button"
            ...
            {chapterOutline.map((group) => (
              <section key={group.chapter}>
                ...
                {group.items.length === 0 ? (
                  <p className="mt-1 text-xs text-[#b8afa4]">
