...
const thoughtButtonClass =
  "w-full rounded-lg px-3 py-2 text-left text-base leading-relaxed text-[#2d2926] hover:bg-[#1e4f4f]/5 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#1e4f4f]/35";

const thoughtButtonActiveClass =
  "w-full rounded-lg bg-[#1e4f4f]/8 px-3 py-2 text-left text-base font-medium leading-relaxed text-[#1f1c19] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#1e4f4f]/35";

function ThoughtActionTrust({ result }: { result: ThoughtActionResult }) {
  if (!result.ok) return null;
  return (
    <div
      className="companion-fade-in mt-3 rounded-xl border border-[#1e4f4f]/25 bg-[#1e4f4f]/[0.07] px-4 py-3 text-base leading-relaxed text-[#2d2926]"
...
export function ClearMyMindReliefClusters({
  ...
  return (
    ...
    >
      <p className="text-xl font-semibold leading-snug text-[#1f1c19]">
        Everything is held.
      </p>

      <p
        className="mt-2 text-lg leading-relaxed text-[#5a5248]"
        role="note"
              ...
              <div
                ...
                onKeyDown={(e) => {
                  if (e.key === "Enter" || e.key === " ") {
                    e.preventDefault();
                    handleClusterTap(cluster.id);
                  }
                }}
                className={`rounded-xl border-2 transition-colors cursor-pointer focus:outline-none focus-visible:ring-2 focus-visible:ring-[#1e4f4f]/35 ${
                  isActive
                    ? "border-[#1e4f4f]/35 bg-white shadow-sm"
                    : "border-transparent bg-white/75 hover:border-[#1e4f4f]/15"
                }`}
              >
                <div className="px-4 py-3 text-left">
                  <div className="flex flex-wrap items-baseline gap-x-2 gap-y-1">
                    ...
                    <span
                      className="text-base tracking-[0.2em] text-[#7c7468]"
                      aria-hidden="true"
                    >
                      {dots}
                      {suffix ? (
                ...
                {isActive ? (
                  ...
                  >
                    <p
                      className="text-base leading-relaxed text-[#1e4f4f]"
                      role="status"
                      aria-live="polite"
      ...
      <p className="mt-4 text-base text-[#6b635a]">Everything is captured.</p>
    </div>
  );
}
